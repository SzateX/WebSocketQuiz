...
import edu.szatkowski.jakub.websocketquizmaven.Responses.EntityRemovedResponse;
...
public class RemoveCategoryCommand implements ICommand{
    ...
    public void execute(Session session, AccountManager accountManager, GameManager gameManager, QuestionsManager questionsManager) {
        ...
        try{
            ...
            Category category = questionsManager.getCategory(this.id);
            questionsManager.removeCategory(category);
            EntityRemovedResponse response = new EntityRemovedResponse(true, category);
