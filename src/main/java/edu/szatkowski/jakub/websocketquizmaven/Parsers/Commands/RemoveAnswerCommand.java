...
public class RemoveAnswerCommand implements ICommand{
    ...
    public void execute(Session session, AccountManager accountManager, GameManager gameManager, QuestionsManager questionsManager) {
        ...
        try{
            ...
            Answer answer = questionsManager.getAnswer(this.id);
            questionsManager.removeAnswer(answer);
            EntityRemovedResponse response = new EntityRemovedResponse(true, answer);
